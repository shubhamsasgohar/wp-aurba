...
class GeotWP_Metaboxes {
	...
	public function add_meta_boxes() {
		...
		foreach ( $post_types as $cpt ) {
			if ( in_array( $cpt, apply_filters( 'geot/exclude/post_types', ['wppopups-templates'] ) ) ) {
				continue;
			}
			if ( ! current_user_can( apply_filters('geot/show_metabox_capabilities', 'edit_posts') ) ) {
				continue;
			}
			add_meta_box(
				'geot-settings',
				...
				$cpt,
				'normal',
				apply_filters('geot/cpt_priority', 'core' )
	...
	public function save_meta_options( $post_id ) {
		...
		if ( ! current_user_can( apply_filters('geot/save_metabox_capabilities', 'edit_post'), $post_id ) ) {
			return $post_id;
		}
	...
	function save_meta_options_geotr_cpt( $post_id ) {
		...
		GeotWP_Helper::save_rules( $post_id, $_POST, 'geotr_rules_global' );
	}
	...
	public function meta_boxes_rules_geotr_cpt( $post, $metabox ) {
		...
		$args = [
			...
			'global_title' => __( "Global Rules", 'geot' ),
			'global_desc'  => __( "The rules added here will apply to every group of rules defined in the rules section above. So if you have multiple \"OR\" groups, it will apply to all them", 'geot' ),
		];
	...
	function save_meta_options_geobl_cpt( $post_id ) {
		...
		GeotWP_Helper::save_rules( $post_id, $_POST, 'geobl_rules_global' );
	}
