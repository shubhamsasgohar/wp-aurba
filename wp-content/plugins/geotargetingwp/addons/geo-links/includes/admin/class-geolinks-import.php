...
class GeotWP_Links_Importer {
	...
	public function __construct() {
		...
		add_action( 'admin_notices', [ $this,'wpb_admin_notice_warn'] );
	...
	function wpb_admin_notice_warn() {
		...
		<p>There was an error with GeoLinks Importer/Exporter</p>
	...
	public function import_save() {
		...
		$mimes_allowed = [ 'application/vnd.ms-excel','text/plain','text/csv','text/tsv','application/csv' ];
	...
	public function import_items( $fields = [] ) {
		...
		$post_name = ! empty( $fields['source_slug'] ) ? sanitize_title( $fields['source_slug'] ) : sanitize_title( $post_title );
		...
		// Dest Default
		$input['dest_default'] = ! empty( $fields['dest_default'] ) ? esc_url_raw( $fields['dest_default'] ) : '';
