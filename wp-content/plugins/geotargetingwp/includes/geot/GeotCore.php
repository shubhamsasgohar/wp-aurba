...
class GeotCore {
	...
	public function __construct() {
		...
		if ( ! is_admin()
			...
			if  ( ! empty( $this->opts['cache_mode'] ) || ! empty( $this->opts['wp_rocket_cache'] ) || apply_filters('geot/enable_rocket_cookies', false ) ) {
				add_action( 'init', [ $this, 'getUserData' ] );
			}
			// disable wprocket cookies when  it's turned off
			if (!  empty( $this->opts['wp_rocket_cache'] ) || apply_filters('geot/enable_rocket_cookies', false ) ) {
	...
	public function target( $key, $args = [] ) {
		...
		if ( count( $places ) > 0 ) {
			...
			foreach ( $places as $p ) {
				...
				if (
					strtolower( check_key($user_place, 'name', '', 'obj' ) ) == strtolower( trim( $p ) )
					|| strtolower( check_key($user_place, 'iso_code', '', 'obj' ) ) == strtolower( trim( $p ) )
					|| strtolower( check_key($user_state, 'name', '', 'obj' ) ) == strtolower( trim( $p ) )
					|| strtolower( check_key($user_state, 'iso_code', '', 'obj' ) ) == strtolower( trim( $p ) )
				) {
					$target = true;
				}
			}
		...
		if ( count( $exclude_places ) > 0 ) {
			// what to return if trying to exclude and user is null
			if ( ! isset( $user_place ) || empty( (array) $user_place ) ) {
				// reset locale
				$this->checkLocale();

				return apply_filters( 'geot/target_' . $key . '/exclude_return_on_user_null', false );
			}
			foreach ( $exclude_places as $ep ) {
				// cant use isset($user_place->name) in here because it will always return false due to object properties
				if (
					strtolower( check_key($user_place, 'name', '', 'obj' ) ) == strtolower( trim( $ep ) )
					|| strtolower( check_key($user_place, 'iso_code', '', 'obj' ) ) == strtolower( trim( $ep ) )
					|| strtolower( check_key($user_state, 'name', '', 'obj' ) ) == strtolower( trim( $ep ) )
					|| strtolower( check_key($user_state, 'iso_code', '', 'obj' ) ) == strtolower( trim( $ep ) )
				) {
					$target = false;
				}
			}
	...
	private function checkLocale( $force_locale = null ) {
		...
		foreach ( get_object_vars( $this->user_data[ $this->cache_key ] ) as $o ) {
			if( is_object($o) && method_exists( $o, 'setDefaultLocale') ) {
	...
	public function getUserData( $params = '', $key = 'ip', $force = false ) {
		...
		try {
			...
			if ( ! apply_filters( 'geot/cancel_query', false ) && wp_using_ext_object_cache() ) {
				$data = wp_cache_get($this->cache_key);
				if ( ! empty($data)  ) {
					$this->user_data[ $this->cache_key ] = $data;
					return $data;
				}

			}	// If we already calculated on session return (if we are not calling by IP & if cache mode (sessions) is turned on)
				elseif ( $this->opts['cache_mode'] && ! empty ( $this->session->get( $this->cache_key ) ) ) {
					$this->user_data[ $this->cache_key ] = new GeotRecord( $this->session->get( $this->cache_key ) );
					$this->checkLocale();
			...
			if ( self::user_whitelisted( $this->ip ) ) {
				return $this->getFallbackCountry();
			}
			// check for crawlers and if rest
			$CD = new CrawlerDetect();
			if ( ( apply_filters('geot/enable_crawl_detection', true) &&  $CD->isCrawler() ) || $this->treatAsBot() || ( is_rest_request() && apply_filters( 'geot/disable_in_rest', true ) ) )  {
				return $this->setData( ! empty( $this->opts['bots_country'] ) ? $this->opts['bots_country'] : 'US' );
			}
			$MD = new \Geot_Mobile_Detect();
			// Hosting database should only work when html5 for mobile is no selected, or it's selected but is not mobile
			if( $this->opts['geolocation'] != 'by_html5_mobile' || ( $this->opts['geolocation'] == 'by_html5_mobile' && ! $MD->isMobile() ) ) {
				// Hosting DB ?
				if (
	 ...
	 */
	public static function user_whitelisted($ip) {
		$ret = false;
		// Ips check
		$settings = geot_settings();
		if ( isset( $settings['fallback_country_ips'] ) && GeotWP_Helper::checkIP( $ip, textarea_to_array( $settings['fallback_country_ips'] ) ) ) {
			$ret = true;
		}
	...
	private function getCountryByIsoCode( $iso_code ) {
		...
		foreach( geot_countries() as $data ) {
			if( $data->iso_code == strtoupper( $iso_code ) ) {
	...
	private function treatAsBot() {
		...
		if ( in_array( $script, [ 'wp-login.php', 'xmlrpc.php', 'wp-cron.php' ] ) ) {
			return apply_filters( 'geot/treat_request_as_bot', true);
		}
		...
		// hardcoded IPS
		include GEOWP_PLUGIN_DIR . '/includes/ip-lists.php';
		...
		if ( false === ( $geot_server_ip = get_transient( 'geot_server_ip' ) ) ) {
			...
			set_transient( 'geot_server_ip', $geot_server_ip, 30 * DAY_IN_SECONDS );
		}
		if ( $geot_server_ip == $this->ip ) {
			return apply_filters( 'geot/treat_request_as_bot', true);
		}

		return apply_filters( 'geot/treat_request_as_bot', false);
	}
	...
	private function cleanResponse( $response ) {
		...
		if ( $this->opts['cache_mode'] ) {
			$this->session->set( $this->cache_key, $response );
		}
	...
	private function targetZip( $places, $exclude_places ) {
		...
		if ( count( $places ) > 0 ) {
			...
			foreach ( $places as $zip ) {
				...
				if ( isset( $user_place->zip ) && strtolower( $user_place->zip ) == strtolower( $zip ) ) {
					$target = true;
				}
			}
		} else {
		...
		if ( count( $exclude_places ) > 0 ) {
			...
			// if we don't have a zip we need to show it or not
			if( empty( $user_place->zip ) ) {
				return apply_filters( 'geot/target_zip/return_on_zip_null', false );
			}
		}
	...
	public function createRocketCookies() {
		...
		if( ! isset( $_COOKIE[$this->opts['cookie_name']] ) ) {
			setcookie( 'geot_rocket_country', apply_filters( 'geot_rocket_country', $country ), 0, '/', '', true );
			setcookie( 'geot_rocket_state', apply_filters( 'geot_rocket_state', $state ), 0, '/', '', true );
			setcookie( 'geot_rocket_city', apply_filters( 'geot_rocket_city', $city ), 0, '/', '', true );
			setcookie( 'STYXKEY_geot_country', apply_filters( 'STYXKEY_geot_country', $country ), 0, '/', '', true );
			if( $cookie = apply_filters( 'geot/custom_rocket_cookie', false ) ) {
				setcookie( $cookie['name'], apply_filters( $cookie['name'], $cookie['value'] ), 0, '/', '', true );
			}
		}
	}
	...
	public function targetRadius( $radius_lat, $radius_lng, $radius_km ) {
		...
		$lat = $this->lat ?? $user_geo->latitude;
		$lng = $this->lng ?? $user_geo->longitude;
		$distance = 0;

		if( is_numeric( $lat ) && is_numeric( $lng ) )
			$distance = $this->getDistance( $lat, $lng, $radius_lat, $radius_lng );
	...
	/**
	 * Saves geo to persistent cache
	 */
	private function saveToOC() {
		// disable save to OC
		if( apply_filters('geot/disable_object_cache', false ) ) {
			return;
		}
		// is persistent object cache enabled?
		if ( ! wp_using_ext_object_cache() ) {
			return;
		}

		wp_cache_set( $this->cache_key, $this->user_data[ $this->cache_key ] );
	}
}
