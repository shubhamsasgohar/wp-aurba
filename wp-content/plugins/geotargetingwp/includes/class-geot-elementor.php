...
class GeotWP_Elementor {
	...
	public function __construct() {
		if( ! defined('ELEMENTOR_VERSION')
			...
			|| apply_filters( 'geot/deactivate_elementor_integration', false )
		) {
			return;
		}
		add_action( 'elementor/init', [ $this, 'register_init' ] );
		...
		add_action( 'elementor/frontend/container/should_render', [ $this, 'is_render' ], 10, 2 );
	...
	public function register_init() {
		...
		//Register Tab
		\Elementor\Controls_Manager::add_tab( 'geot', esc_html__( 'Geotargeting', 'geot' ) );

		global $geot_widgets;
		$GLOBALS['geot_widgets'] = [];

		require_once GEOWP_PLUGIN_DIR . 'includes/elementor/elementor-geot-updater.php';
	...
	public function register_controls( $control, $section_id, $args ) {
		...
		if( $control instanceof Elementor\Modules\Library\Documents\Section )
			return;

		if( ! class_exists( 'Elementor_GeoCountry' ) ||
			! class_exists( 'Elementor_GeoCity' ) ||
			! class_exists( 'Elementor_GeoState' ) ||
			! class_exists( 'Elementor_GeoZipcode' ) ||
			! class_exists( 'Elementor_GeoRadius' )
		) return;

		if ( ! is_array($GLOBALS['geot_widgets']) || ! in_array( $control->get_name(), $GLOBALS['geot_widgets'] ) ) {
