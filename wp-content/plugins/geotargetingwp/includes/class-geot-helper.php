...
class GeotWP_Helper {
	...
	public static function get_geotarget_posts( $pid = null ) {
		...
		self::$_geotarget_posts = wp_cache_get('geot.targeted_posts'.$pid, 'geot', false, $found);

		// If found in the cache return even if empty to avoid multiple calls
		if ( $found && ! apply_filters('geot/cancel_get_geotargeted_posts', false ) && empty( $_GET['geot_debug'] ) ) {
			return self::$_geotarget_posts;
		}

		if( apply_filters('geot/cancel_get_geotargeted_posts', false ) ) {
			return [];
		}
		...
		self::$_geotarget_posts = $wpdb->get_results( $sql );

		wp_cache_set('geot.targeted_posts'.$pid, self::$_geotarget_posts,'geot', 3600 );
	...
	public static function ajax_render_operator( $options = [] ) {
		...
		if ( $is_ajax ) {
			...
			$name = strpos( $options['name'], 'global' ) !== false ? 'geot_global_rules' : 'geot_rules';
			$options['name'] = $name . '[' . $options['group_id'] . '][' . $options['rule_id'] . '][operator]';
		} else {
	...
	public static function ajax_render_options( $options = [] ) {

		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'geot_nonce' ) ) {
			$options = $_POST;
		}
		$choices = [];
		add_filter( 'posts_where', [self::class ,'title_filter'], 10, 2 );
		switch ($options['param']) {
			case "post" :
				...
				foreach ( apply_filters( 'geot/exclude/post_types', ['acf-field','oembed_cache','wpcode','custom_css','custom-css-js','acf-field-group'] ) as $ptype ) {
					...
					foreach ( $post_types as $post_type ) {
						...
						if ( $posts ) {
							foreach ( $posts as $post ) {
								...
								$choices[ ] = ['optgroup' => $post_type, 'id' => $post->ID, 'name' => "[{$post_type}](#".$post->ID.") ".$title , 'label' => $post_type];
							}
							// foreach($posts as $post)
						}
						// if( $posts )
			...
			case "page":
				...
				if ( $posts ) {
					...
					foreach ( $posts as $page ) {
						...
						$choices[] = ['id' => $page->ID, 'name' => "(#".$page->ID.") ".$title];
					}
				}
				break;
		}
		remove_filter( 'posts_where', [self::class ,'title_filter'], 10, 2 );
	...
	static function print_selectize( $options, $choices ) {
		// value must be array
		...
		if ( is_array( $choices ) ) {
			...
			foreach ( $choices as $k => $v ) {
				if ( is_array( $v ) ) {
					$total_choices = count($v) > $total_choices ? count($v) : $total_choices;
					$optgroup = true;
				}
			}
		}

		echo '<select data-param="'.$options['param'].'" id="geot_rule_' . $options['group_id'] . '_rule_' . $options['rule_id'] . '" class="selectize" name="' . $options['name'] . '">';
		echo '<option value="" >'. __('Choose one','geot') . '</option>';
				...
				if ( isset( $optgroup ) ) {
					...
					if ( is_array( $value ) ) {
						foreach ( $value as $id => $label ) {
							...
							if( !empty($selected) || $total_choices < 100 ) {
								echo '<option value="' . $id . '" ' . $selected . '>['.$key.'](#'.$id.') ' . $label . '</option>';
							}
						}
					}
	...
	public static function ajax_render_rules( $options = [] ) {
		...
		if ( $is_ajax ) {
			$options         = array_merge( $defaults, $_POST );
			$name = strpos( $options['name'], 'global' ) !== false ? 'geot_global_rules' : 'geot_rules';
			$options['name'] = $name . '[' . $options['group_id'] . '][' . $options['rule_id'] . '][value]';
		}
		...
		switch ( $options['param'] ) {
			...
			case "page":
				...
				if ( $posts ) {
					// sort into hierachial order!
					if ( empty($options['value']) && is_post_type_hierarchical( $post_type ) ) {
						$posts = get_page_children( 0, $posts );
					}
	...
	public static function save_rules( $post_id, $input, $meta_key = 'geot_rules' ) {
		...
		if ( ( ! isset( $input['geot_rules'] ) && ! isset( $input['geot_global_rules'] ) ) || ( ! is_array( $input['geot_rules'] ) &&  ! is_array( $input['geot_global_rules'] ) ) ) {
			return;
		}
		...
		$data = 'geotr_rules_global' == $meta_key || 'geobl_rules_global' == $meta_key  ?  $input['geot_global_rules'] : $input['geot_rules'];

		// clean array keys
		$groups = empty( $data ) ? [] : array_values( $data );
		unset( $data );
	...
	public static function html_rules( $post, $meta_key, $args = [] ) {
		...
		$global_groups = apply_filters( 'geot/metaboxes/get_rules', GeotWP_Helper::get_rules( $post->ID, $meta_key . '_global' ), $post->ID );
	...
	public static function get_rules( $id, $meta_key = 'geot_rules' ) {
		...
		if( strpos( $meta_key, 'global') !== false ) {
			$defaults[0][0]['param'] = '';
			$defaults[0][0]['value'] = '';
			$defaults[0][0]['operator'] = '';
		}
