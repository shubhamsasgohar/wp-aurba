...
class GeotWP_Ajax {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_geot/field_group/render_options', [ 'GeotWP_Helper', 'ajax_render_options' ], 1 );
		...
		add_filter( 'geot/ajax/geo_link', [ $this, 'geo_link' ] );
	}
	...
	public function geot_ajax() {
		...
		add_filter( 'weglot_ajax_replace_urls',[ $this, 'weglot'] );
		echo json_encode( $result );
		die();
	}

	/**
	 * Weglot decided to translate everything including ajax calls, breaking our redirect plugin
	 * So we need to remove url from their automatic keys
	 * @param $keys
	 *
	 * @return void
	 */
	public function weglot($keys) {
	...
	private function get_geotargeted_posts() {
		...
		if( isset( $settings['disable_remove_post'] ) && '1' == $settings['disable_remove_post'] ) {
			$geot_posts = isset( $this->data['vars']['is_singular'] ) &&  $this->data['vars']['is_singular'] ? GeotWP_Helper::get_geotarget_posts( $this->data['vars']['pid'] ?? '' ) : [];
		} else {
	...
	public function geo_link( $geot ) {
		$opts = unserialize( base64_decode( $geot['filter'] ) );

		if( ! isset( $opts['id'] ) ) {
			return false;
		}
		return Geol_Redirects::get_geo_link_html( $opts['id'], $opts );
	}
