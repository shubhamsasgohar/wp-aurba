...
class GeotWP_R_ules {
	...
	* @since 1.0.0
	*/
	public static function is_ok( $rules = [], $global_rules = [] ) {

		if ( empty( $rules ) && empty( $global_rules ) ) {
			return false;
		}

		$do_redirect = self::check_group( $rules );
		// if we have a redirect in groups, check the global rules
		if( $do_redirect && ! empty( $global_rules ) ) {
			$do_redirect = self::check_group( $global_rules );
		}
	...
	public static function set_rules_fields() {
		...
		add_action( 'geot/rules/print_post_field', [ 'GeotWP_Helper', 'print_selectize' ], 10, 2 );
		...
		// Page
		add_action( 'geot/rules/print_page_field', [ 'GeotWP_Helper', 'print_selectize' ], 10, 2 );
	...
	public static function get_rules_choices() {
		...
		// WPML or Polylang
		if ( function_exists( 'icl_object_id' ) || function_exists( 'pll_current_language' ) || class_exists('TRP_Translate_Press') ) {
			$choices[ __( 'Other', 'geot' ) ]['language'] = __( 'Language', 'geot' );
		}
	...
	*/
	...
	public static function rule_match_custom_url( $rule ) {
		...
		// remove query string for comparision
		$current_url = self::cleanUrl( self::$current_url );
		$custom_url  = self::cleanUrl( $rule['value'] );

		// remove trailing /
		$custom_url  = untrailingslashit( preg_replace( '#^https?://#', '', $custom_url ) );
		$current_url = untrailingslashit( preg_replace( '#^https?://#', '', $current_url ) );
	...
	public static function rule_match_language( $rule ) {
		...
		if( class_exists('TRP_Translate_Press') ) {
			$lang = isset($GLOBALS['TRP_LANGUAGE'] ) ? $GLOBALS['TRP_LANGUAGE'] : '';
		}
	...
	*/
	 ...
	 * @return bool|mixed
	 */
	private static function check_group( $rules ) {
		$do_redirect = false;
	...
	/**
	 * Remove query string and hashcode
	 * @param $url
	 *
